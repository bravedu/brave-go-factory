...
func (bm BodyMap) MarshalXML(e *xml.Encoder, start xml.StartElement) (err error) {
	...
	start.Name = xml.Name{Space: constants.NULL, Local: "xml"}
	if err = e.EncodeToken(start); err != nil {
		return
	}
	for k := range bm {
		if v := bm.GetString(k); v != constants.NULL {
...
func (bm BodyMap) CheckEmptyError(keys ...string) error {
	...
	if len(emptyKeys) > 0 {
		return fmt.Errorf("[%w], %v", constants.MissParamErr, strings.Join(emptyKeys, ", "))
	}
	return nil
}
