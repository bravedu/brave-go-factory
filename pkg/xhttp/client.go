...
func (c *Client) SendStruct(v interface{}) (client *Client) {
	...
	if err != nil {
		c.err = fmt.Errorf("[%w]: %v, value: %v", constants.MarshalErr, err, v)
		return c
	}
	...
	case TypeXML, TypeUrlencoded, TypeForm, TypeFormData:
		...
		if err = json.Unmarshal(bs, &body); err != nil {
			c.err = fmt.Errorf("[%w]: %v, bytes: %s", constants.UnmarshalErr, err, string(bs))
			return c
		}
...
func (c *Client) SendBodyMap(bm map[string]interface{}) (client *Client) {
	...
	case TypeJSON:
		...
		if err != nil {
			c.err = fmt.Errorf("[%w]: %v, value: %v", constants.MarshalErr, err, bm)
			return c
		}
		c.jsonByte = bs
	case TypeXML, TypeUrlencoded, TypeForm, TypeFormData:
...
func (c *Client) EndStruct(ctx context.Context, v interface{}) (res *http.Response, err error) {
	...
	case string(TypeJSON):
		...
		if err != nil {
			return nil, fmt.Errorf("[%w]: %v, bytes: %s", constants.UnmarshalErr, err, string(bs))
		}
		return res, nil
	case string(TypeXML):
