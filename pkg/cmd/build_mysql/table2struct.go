...
func (t *Table2Struct) getColumns(table ...string) (tableColumns map[string][]column, err error) {
	...
	// sql
	var sqlStr = `SELECT COLUMN_NAME,DATA_TYPE,IS_NULLABLE,TABLE_NAME,COLUMN_COMMENT,COLUMN_TYPE
	...
	for rows.Next() {
		col := column{}
		err = rows.Scan(&col.ColumnName, &col.Type, &col.Nullable, &col.TableName, &col.ColumnComment, &col.ColumnType)
		...
		//如果字段包含time && 是int型,则设立为bigint
		if col.Type == "int" && strings.Contains(strings.ToLower(col.ColumnName), "time") {
			col.Type = "bigint"
		}
		//判断是否存在unsigned--不启动表示
